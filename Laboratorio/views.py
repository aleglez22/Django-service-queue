...
class Contador(TemplateView):
    def get(self, request, **kwargs):
        return render(request, 'Laboratorio/contador.html', context=None)
...
def SubirArchivo(request):  
    ...
    if request.method == 'POST' and request.FILES.get('exampleInputFile'):
        ...
        if str(myfile.name).endswith(".ino"):
            ...
            comando='cp {0} {1} '.format(myfile.name, respaldosDir)
            print( "comando = "+ str(comando))
            subprocess.Popen(comando, shell=True, cwd=wdir) 
            '''
            import datetime
            import shutil
            respaldosDir = 'Respaldos'
